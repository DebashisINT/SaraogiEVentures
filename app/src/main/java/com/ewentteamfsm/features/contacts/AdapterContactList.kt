    ...
    inner class ContactListViewHolder(itemView: View):RecyclerView.ViewHolder(itemView){
        fun bindItems(){
            itemView.apply {
                ...
                tv_row_cont_list_added_dt_ti.text = AppUtils.getFormatedDateNew(added_date,"yyyy-mm-dd","dd-mm-yyyy")+"   "+added_time
                tv_row_cont_list_cont_type.text = if(shopL.get(adapterPosition).crm_type.equals("")) "N/A" else shopL.get(adapterPosition).crm_type
                tv_row_cont_list_cont_source.text = if(shopL.get(adapterPosition).crm_source.equals("")) "N/A" else  shopL.get(adapterPosition).crm_source
                tv_row_cont_list_cont_stage.text = if(shopL.get(adapterPosition).crm_stage.equals("")) "N/A" else  shopL.get(adapterPosition).crm_stage
                tv_row_cont_list_cont_assign_to.text = if(shopL.get(adapterPosition).crm_assignTo.equals("")) "N/A" else  shopL.get(adapterPosition).crm_assignTo
                tv_row_cont_list_cont_status.text = if(shopL.get(adapterPosition).crm_status.equals("")) "N/A" else  shopL.get(adapterPosition).crm_status
                tv_row_cont_list_expctd_sales_value.text = if(shopL.get(adapterPosition).amount.equals("")) "0.00" else  shopL.get(adapterPosition).amount

                tv_row_cont_list_cont_number_whatsapp.text = shopL.get(adapterPosition).ownerContactNumber
                tv_row_cont_list_cont_number_email.text = if(shopL.get(adapterPosition).ownerEmailId.equals("")) "N/A" else shopL.get(adapterPosition).ownerEmailId

                tv_row_cont_list_added_addition_source.text = if(shopL.get(adapterPosition).crm_saved_from.equals("")) "N/A" else shopL.get(adapterPosition).crm_saved_from
